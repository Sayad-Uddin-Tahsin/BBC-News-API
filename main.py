...
async def head(language, type):
    if str(language).lower() in urls:
        ...
        langs = [i.lower() for i in urls]
        return flask.Response(json.dumps({"status": 400, "error": "Invalid Language!", "languages": langs}, ensure_ascii=False).encode('utf8'), mimetype="application/json", status=400)
    if str(type) == 'news':
        try:
            start = int(time.time())
            r = session.get(url)
